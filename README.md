...
**Keybot 图标显示在跑，但按键完全没反应，像卡死了一样**

某些规则（比如 Terminal/Edge 的 Ctrl+C）需要查询"当前有没有选中文本"，这个查询要跨进程问一下目标 App，如果目标 App 恰好卡住/无响应（比如网页很重的 Edge 标签页），查询会一直等它回应。这个查询和所有按键事件挤在同一条处理队列里，一等住就等于把全系统键盘输入都拖死——不是 Keybot 自己卡死，是被别的 App 拖累的。已经给这类查询加了 0.2 秒的超时上限（`AXUIElementSetMessagingTimeout`），超时就当查不到处理，不会无限期等待。如果还遇到类似"看着在跑但完全没反应"的情况，先检查是不是有 App 处于无响应状态。
